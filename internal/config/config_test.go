...
func TestLoadConfig_NotFoundInStartDir(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(startDir, ".error-log-or-return.yml")); err == nil {
...
func TestLoadConfig_FoundInTempDir(t *testing.T) {
  ...
  - "vendor/**"
  - "**/*.pb.go"`)
	if err := os.WriteFile(".error-log-or-return.yml", content, 0644); err != nil {
		t.Fatal(err)
	}

	// Check that file exists
	if _, err := os.Stat(".error-log-or-return.yml"); err != nil {
...
func TestLoadConfig_PermissionDenied(t *testing.T) {
	...
	content := []byte(`ignore:
  - "vendor/**"`)
	configPath := filepath.Join(noAccessDir, ".error-log-or-return.yml")
